...
class utility_functions:
    ...
    def output_transform(train_y, apply_transform):
        ...
        '''
        if apply_transform == True and train_y.shape[1] > 1:
            train_y[:, 0] = -torch.log(1/train_y[:, 0])
            train_y[:, 1] = -train_y[:, 1]
        
        elif apply_transform == True and train_y.shape[1] == 1:
            train_y[:, 0] = train_y[:, 0]
    ...
    def densification_filtering(train_x, train_y, ea_mode):
        ...
        '''
        # Check if ea_mode is provided
        if ea_mode is not None:
            # Iterate over ea_mode and modify train_y accordingly
    ...
    def initial_data_to_csv(path_to_spinodal_resources, hydra_output_directory, input_columns, output_columns, path_to_MOO, initial_data_file_name, tkwargs, apply_transform):
        ...
        # Check if 'EA_Mode' is in output_columns
        ...
        # Create the output array excluding 'EA_Mode'
        ...
        # Recreate the DataFrame with transformed outputs and add back 'EA_Mode' if it was excluded
        ...
        # Convert numerical input and output data to numpy arrays
    ...
    def append_to_optimization_history_csv(optimisation_file_path, data, input_data, input_columns, output_columns):
        '''
        ...
        if data == 'new_x':
            ...
            df.loc[df.index[-1], output_columns] = data_np #if there is existing data df = pd.concat([df, new_row[output_columns]], ignore_index=True)
            df.to_csv(optimisation_file_path, index=False)
    ...
    def sample_sobol_sequence(problem, num_samples, batch_size, seed):

        samples = draw_sobol_samples(bounds=problem.bounds, n=num_samples, q=batch_size, seed=seed).cpu().detach().numpy()
